...
Quikdown excels in being extremely small, secure by default, bidirectional (HTML ↔ Markdown), and rich in technical fence support. A desktop version called **qdedit** (built with Tauri) is currently in progress, featuring offline/standalone fences, CLI support, automation options, standalone GUI editor, and LLM integration.
...
| **File Management**              | Excellent (qdedit: native + CLI)     | Excellent (vaults, folders, search)  | Good (local files + outline)        | Excellent (project/folder)          |
...
| **Desktop App**                  | **Yes (qdedit in progress - Tauri)** | Yes (Windows/Mac/Linux + mobile)    | Yes (Windows/Mac/Linux)             | Yes                                 |
| **Export Options**               | HTML, PDF, CLI + automation (qdedit) | Many via plugins                     | PDF, HTML, Word (via Pandoc)        | Flexible via extensions             |
...
This would immediately make qdedit stand out from Obsidian (more rigid panes) and Typora (no CLI/automation). Nail the core writing feel first — everything else (CLI, LLM, etc.) becomes a massive bonus on top of a great editor.
...
Focusing on these will help qdedit quickly become a top-tier Markdown editor that combines the best of lightweight design, technical richness, and modern features.
